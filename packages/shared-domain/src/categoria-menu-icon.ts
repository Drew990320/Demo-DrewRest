 ...
 */
export const CATEGORIA_MENU_ICON_CATEGORIAS = [
  {
    id: 'general',
    titulo: 'General',
    iconos: [
...
function flattenCategoriaIconCatalog(): { id: string; label: string }[] {
  const seen = new Set<string>();
  const out: { id: string; label: string }[] = [];
  for (const cat of CATEGORIA_MENU_ICON_CATEGORIAS) {
...
export type CategoriaMenuIconId =
  (typeof CATEGORIA_MENU_ICON_CATEGORIAS)[number]['iconos'][number]['id'];

/** Lista plana de iconos permitidos (derivada del catálogo categorizado). */
export const CATEGORIA_MENU_ICONOS: ReadonlyArray<{
  id: CategoriaMenuIconId;
  label: string;
}> = flattenCategoriaIconCatalog() as { id: CategoriaMenuIconId; label: string }[];

export const CATEGORIA_MENU_ICON_IDS: CategoriaMenuIconId[] =
  CATEGORIA_MENU_ICONOS.map((i) => i.id);

const ICON_SET = new Set<string>(CATEGORIA_MENU_ICON_IDS);

/** Iconos guardados antes de corregir el catálogo. */
const ICONOS_LEGACY: Record<string, CategoriaMenuIconId> = {
...
export function esIconoCategoriaMenuValido(
  icono: string | null | undefined,
): icono is CategoriaMenuIconId {
  return typeof icono === 'string' && ICON_SET.has(icono);
}

/** Sugiere icono según el nombre (cuando el admin no eligió uno). */
export function inferirIconoCategoriaDesdeNombre(nombre: string): CategoriaMenuIconId {
  ...
  if (n.includes('té') || n.includes('te ')) return 'tea-outline';
  if (n.includes('vino')) return 'glass-wine';
  if (n.includes('cóctel') || n.includes('coctel')) return 'glass-cocktail';
  if (n.includes('postre') || n.includes('dulce')) return 'cake-variant';
  if (n.includes('helado')) return 'ice-cream';
  if (n.includes('fruta')) return 'fruit-cherries';
  if (n.includes('galleta') || n.includes('cookie')) return 'cookie';
  if (n.includes('empaque') || n.includes('llevar')) return 'food-takeout-box-outline';
  ...
  if (n.includes('entrada') || n.includes('adicional')) return 'silverware-fork-knife';
  if (n.includes('ensalada')) return 'bowl-mix-outline';
  if (n.includes('pizza')) return 'pizza';
  if (n.includes('pasta') || n.includes('espaguet')) return 'noodles';
  if (n.includes('arroz')) return 'rice';
  if (n.includes('taco')) return 'taco';
  if (n.includes('hamburg')) return 'hamburger';
  if (n.includes('pan')) return 'bread-slice-outline';
  if (n.includes('queso')) return 'cheese';
  if (n.includes('pescad') || n.includes('marisc') || n.includes('camar')) return 'fish';
  if (n.includes('cerdo') || n.includes('costilla') || n.includes('bondiola')) return 'pig';
  if (n.includes('pollo') || n.includes('pechuga') || n.includes('nugget')) return 'food-drumstick-outline';
  if (n.includes('res') || n.includes('mixto') || n.includes('parrillada')) return 'food-steak';
...
export function normalizarIconoMenuGuardado(
  raw: string | null | undefined,
  nombreFallback?: string,
): CategoriaMenuIconId | null {
  if (raw == null || raw === '') {
  ...
  if (esIconoCategoriaMenuValido(id)) return id;
  return nombreFallback
    ? inferirIconoCategoriaDesdeNombre(nombreFallback)
    : null;
}

export function resolverIconoCategoriaMenu(
  nombre: string,
  iconoGuardado?: string | null,
): CategoriaMenuIconId {
  return (
    normalizarIconoMenuGuardado(iconoGuardado, nombre) ??
