...
export const GRUPOS_VISTA_PALETA_APP = [
  {
    titulo: 'Marca',
    keys: [
      'primary',
...
function normalizePrimary(hex: string): string {
  ...
  return VISUAL_COLOR_DEFAULTS.primary;
}
...
export function modoPaletaVisual(
...
function contrasteRelativo(fg: string, bg: string): number {
...
export function normalizarContrastePaleta(
  palette: Record<VisualColorKey, string>,
): Record<VisualColorKey, string> {
  ...
  if (ratioTexto >= 4.5 && ratioMuted >= 3) return palette;
  ...
  return {
    ...
    border:
      contrasteRelativo(palette.border, palette.background) < 2
        ? sugerida.border
        : palette.border,
    surface:
      modo === 'oscuro' &&
...
export function derivarColoresSemanticos(
  palette: Record<VisualColorKey, string>,
  modo: ModoTemaVisual = modoPaletaVisual(palette),
): ColoresSemanticosDerivados {
  const brand = parseHex(palette.primary);
  const brandH = brand ? rgbToHsl(brand.r, brand.g, brand.b).h : 12;
  const oscuro = modo === 'oscuro';
  const infoH =
    brandH >= 160 && brandH <= 260
      ? (brandH + 12) % 360
      : (brandH + 205) % 360;
...
export function generarPaletaClaraDesdePrincipal(
  primaryHex: string,
): Record<VisualColorKey, string> {
  const parsed = parseHex(primaryHex);
  if (!parsed) return { ...VISUAL_COLOR_DEFAULTS };
  ...
  return {
    primary,
    primary_dark: hslToHex(
      h,
      clamp(sBrand * 1.05, 30, 82),
      clamp(lBrand - 16, 28, 52),
    ),
    secondary: hslToHex(
      (h + 32) % 360,
      clamp(sBrand * 0.92, 25, 75),
      clamp(lBrand + 6, 40, 72),
    ),
...
export function generarPaletaOscuraDesdePrincipal(
  primaryHex: string,
): Record<VisualColorKey, string> {
  const parsed = parseHex(primaryHex);
  if (!parsed) return { ...VISUAL_COLOR_DEFAULTS };
  ...
  return {
    ...
    primary_dark: hslToHex(
      h,
      clamp(sBrand * 1.08, 34, 88),
      clamp(lBrand - 10, 32, 56),
    ),
    secondary: hslToHex(
      (h + 32) % 360,
      clamp(sBrand * 0.9, 28, 72),
      clamp(lBrand + 2, 42, 62),
    ),
...
export function generarSugerenciasTemaDesdePrincipal(
  primaryHex: string,
): SugerenciaTemaVisual {
  return {
...
export function generarPaletaDesdePrincipal(
  primaryHex: string,
): Record<VisualColorKey, string> {
