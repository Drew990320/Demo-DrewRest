 ...
 */
export function asignarCantidadesParaSubtotal(
  lineas: LineaAsignableCobro[],
  subtotalObjetivo: number,
): Record<number, number> {
  const out: Record<number, number> = {};
  const objetivo = Math.round(subtotalObjetivo);
  if (objetivo <= 0 || lineas.length === 0) return out;

  let restante = objetivo;
  const porPrecioDesc = [...lineas].sort(
  ...
  if (restante > 0) {
    const porPrecioAsc = [...lineas].sort(
    ...
    for (const l of porPrecioAsc) {
      ...
      if (pu <= 0) continue;
      const ya = out[l.id_detalle] ?? 0;
      if (ya >= l.cantidad_pendiente) continue;
