# Copia el API compilado a DrewRest\api\ para usar con DrewRest\inicio.bat
# Uso (desde la raiz del repo, carpeta App):
#   npm run DrewRest:Empaquetar-Api
#   powershell -ExecutionPolicy Bypass -File scripts\empaquetar-api-drewrest.ps1
...
$ApiDst = Join-Path $DrewRestRoot "api"
...
if (-not (Test-Path $MainJs)) {
  ...
  Write-Host "  npm run DrewRest:Empaquetar-Api" -ForegroundColor Cyan
...
$imagesSrc = Join-Path $RepoRoot "images"
if (Test-Path $imagesSrc) {
  Copy-Item -Path $imagesSrc -Destination (Join-Path $ApiDst "images") -Recurse -Force
  $imagesSibling = Join-Path (Split-Path $ApiDst -Parent) "images"
  if (-not (Test-Path $imagesSibling)) {
    Copy-Item -Path $imagesSrc -Destination $imagesSibling -Recurse -Force
  }
}
...
Write-Host ""
Write-Host "Listo: DrewRest\api" -ForegroundColor Green
Write-Host "Siguiente: npm run DrewRest:Empaquetar-Web  (o DrewRest:Empaquetar para todo)" -ForegroundColor Green
Write-Host "Luego en el PC destino: DrewRest\inicio.bat" -ForegroundColor Green
Write-Host ""
