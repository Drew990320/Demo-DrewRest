/**
 * Matriz de escenarios de cobro (modalidad × forma de pago × estado × bordes).
 * Lógica pura en @la-reserva/shared-domain/cobro-invariantes.
 */
import {
  estadoCobroPedido,
...
} from '@la-reserva/shared-domain/cobro-invariantes';
import {
...
} from '@la-reserva/shared-domain/cobro-parcial';
import {
  repartirMontoEnCop,
...
describe('matriz cobro — mixto montos exactos (causa raíz permutación)', () => {
  ...
  it('cuando montos ya coinciden no fuerza split precio', () => {
    expect(
      necesitaSplitPrecioMixto({
        efectivoFactura: 24_000,
        transferenciaFactura: 24_000,
        solicitudesEfectivoLen: 1,
        solicitudesTransferenciaLen: 1,
        totalLegEfectivo: 24_000,
        totalLegTransferencia: 24_000,
      }),
    ).toBe(false);
  });
...
describe('matriz cobro — reversa de tanda', () => {
  it('facturasDeTandaCobro aísla mixto sin tocar otras tandas', () => {
    const facturas = [
      { id_factura: 1, metodo_pago: 'efectivo', total: 16_000 },
      {
        id_factura: 2,
    ...
    const tandaMixto = facturasDeTandaCobro(facturas, 2);
    expect(tandaMixto.map((f) => f.id_factura).sort()).toEqual([2, 3]);
    const resto = facturas.filter(
      (f) => !tandaMixto.some((t) => t.id_factura === f.id_factura),
    );
    expect(resto.map((f) => f.id_factura)).toEqual([1]);
    expect(totalFacturadoPedido(resto)).toBe(16_000);
  });
});
...
describe('matriz cobro — bordes', () => {
  ...
  it('doble cobro de ítem: pedidoCobroCompleto solo si todos marcados una vez', () => {
    expect(
      pedidoCobroCompleto([{ cobrado: true }, { cobrado: true }]),
    ).toBe(true);
